...
class UniformSampling(ConfidenceEstimationStrategy):
    ...
    def generate_samples(self, eval_points, n, img_shape):
        noised_eval_im = eval_points + \
                       tf.random_uniform(tf.shape(eval_points), minval=-1, \
...
class GaussianNoise(ConfidenceEstimationStrategy):
    ...
    def generate_samples(self, eval_points, n, img_shape):
        noised_eval_im = tf.clip_by_value(eval_points +
                                          tf.random_normal(tf.shape(eval_points)) * self.c , 0, 1)
...
class PoissonNoise(ConfidenceEstimationStrategy):
    ...
    def generate_samples(self, eval_points, n, img_shape):
        noised_eval_im = tf.clip_by_value(eval_points +
                                          tf.random_poisson(self.lamda, tf.shape(eval_points)), 0, 1)
...
class ImageTransformation(ConfidenceEstimationStrategy):
    ...
    def generate_samples(self, eval_points, n, img_shape):
        ...
        points = tf.reshape(transformed_points, tf.shape(eval_points))
...
class ImageAdjustment(ConfidenceEstimationStrategy):
    ...
    def generate_samples(self, eval_points, n, img_shape):
        ...
        points = tf.reshape(adjusted_points, tf.shape(eval_points))
        if self.noise is not None:
            points = points + \
                     tf.random_uniform(tf.shape(eval_points), minval=-1, \
                                       maxval=1) * self.noise
...
def get_transform(strat, strat_param):
    if strat == "uniform":
